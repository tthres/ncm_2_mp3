...
* code: 要解码的数据
*/
static unsigned char* base64_decode(unsigned char* code, int len, int* actLen)
{
...
int readFileData(const char* fileName)
{
    ...
    char* fileNameCopy = strdup(fileName);  // 因为dirname可能会修改传入的字符串，我们传入其副本
    char* directoryPath = dirname(fileNameCopy);

    // 我们需要考虑路径分隔符和终止的空字符
    char* saveFileName =(char*)malloc(strlen(directoryPath) + 1 + strlen(musicName) + strlen(artist) + strlen(format)+5);
    sprintf(saveFileName, "%s/%s - %s.%s", directoryPath, artist, musicName, format);
